...
protected:
	...
	UPROPERTY()
	TObjectPtr<class USkeletalMeshComponent> LeaderPoseComponent;

	/**We cache this so when an item is removed we can set it back to the default*/
	UPROPERTY()
	TMap<EEquippableSlot, TObjectPtr<USkeletalMesh>> DefaultClothing;
	...
	UPROPERTY()
	TMap<EEquippableSlot, TObjectPtr<USkeletalMeshComponent>> EquippableComponents;

	/**All of the items that are currently equipped are stored in here*/
	UPROPERTY(BlueprintReadOnly, Category = "Equipment")
	TMap<EEquippableSlot, TObjectPtr<UEquippableItem>> EquippedItems;


public:

	FORCEINLINE class USkeletalMeshComponent* GetLeaderPoseComponent() const {return LeaderPoseComponent;};
	FORCEINLINE TMap<EEquippableSlot, TObjectPtr<USkeletalMeshComponent>> GetEquippableComponents() const {return EquippableComponents;};
	FORCEINLINE TMap<EEquippableSlot, TObjectPtr<USkeletalMesh>> GetDefaultClothing() const {return DefaultClothing;};
