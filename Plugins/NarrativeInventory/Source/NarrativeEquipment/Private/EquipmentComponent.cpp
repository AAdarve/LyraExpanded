...
void UEquipmentComponent::Initialize(TMap<EEquippableSlot, USkeletalMeshComponent*> ClothingMeshes, class USkeletalMeshComponent* InLeaderPoseComponent)
{
	...
	EquippableComponents.Empty(ClothingMeshes.Num());
	for (const TPair<EEquippableSlot, USkeletalMeshComponent*>& ClothingMeshPair : ClothingMeshes)
	{
		EquippableComponents.Add(ClothingMeshPair.Key, ClothingMeshPair.Value);
	}
